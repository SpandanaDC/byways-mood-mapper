...
// Trip Type options (merged with Occasion) - Emojis Removed
const tripTypes = [
...
// --- NEW EXPANDED ITINERARY DATABASE ---
// Based on Google Search results for real Bangalore itineraries
// Note: In a real app, these would likely link by ID to the Places collection.
// For now, we use names to search or display.
...
const getItinerary = (tripType: string, budget: string) => {
  // Find a matching plan
  let plan = itineraryDatabase.find(item => item.type === tripType && item.budget === budget);

  // If no exact match, try matching just type
  if (!plan) {
    plan = itineraryDatabase.find(item => item.type === tripType);
  }

  // If still no match, return default
  if (!plan) {
    plan = itineraryDatabase.find(item => item.type === "default");
  }
...
const PlanDay = () => {
  ...
  const handleNext = () => {
    if (currentStep < 2) {
      ...
      // Generate itinerary
      const plan = getItinerary(tripType, budget); // Use new state
      setItinerary(plan);
      setCurrentStep(3);
    }
  };
